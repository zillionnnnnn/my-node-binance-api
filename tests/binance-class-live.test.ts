...
describe('Prices', function () {
    it('Checks the price of ETHUSDT', async function () {
        const ticker = await binance.prices('ETHUSDT')
        ethusdtPrice = ticker.ETHUSDT;
        assert(typeof (ticker) === 'object', WARN_SHOULD_BE_OBJ);
        assert(ticker !== null, WARN_SHOULD_BE_NOT_NULL);
        //done()
    }).timeout(TIMEOUT);

    it('Checks the price of ETHUSDT contract', async function () {
        const ticker = await futuresBinance.futuresPrices('ETHUSDT')
        assert(typeof (ticker) === 'object', WARN_SHOULD_BE_OBJ);
        assert(ticker !== null, WARN_SHOULD_BE_NOT_NULL);
        assert(Object.prototype.hasOwnProperty.call(ticker, 'ETHUSDT'), WARN_SHOULD_HAVE_KEY + 'ETHUSDT');
        //done()
    }).timeout(TIMEOUT);
});
...
describe('All Futures Prices', function () {
    it('Checks the prices of coin pairs', async function () {
        const ticker = await futuresBinance.futuresPrices();
        assert(typeof (ticker) === 'object', WARN_SHOULD_BE_OBJ);
        assert(ticker !== null, WARN_SHOULD_BE_NOT_NULL);
    }).timeout(TIMEOUT);
});
...
describe('MarketBuy/Sell WithCost', function () {
    it('Attempt to buy ETH using the cost', async function () {
        let quantity = 100; // usd
        const res = await binance.marketBuyWithCost('ETHUSDT', quantity)
        assert(res['orderId'] !== undefined)
    }).timeout(TIMEOUT);
    it('Attempt to sell ETH using the cost', async function () {
        let quantity = 100; // usd
        const res = await binance.marketSellWithCost('ETHUSDT', quantity)
        assert(res['orderId'] !== undefined)
    }).timeout(TIMEOUT);
});
...
describe('Futures Cancel order', function () {
    it('Attempt to cancel an order', async function () {
        let orderid = futuresOrderId ?? "123123";
        try {
            const res = await futuresBinance.futuresCancel('ETHUSDT', orderid)
            assert(res !== null, WARN_SHOULD_BE_NOT_NULL);
        } catch (e) {
            assert(e.toString().includes('{"code":-2011,"msg":"Unknown order sent."}'));
        }
